...
namespace ESCALATE {
    ...
    class FlexAddress {
        ...
        void parseIP() {
            ...
            if (flag == IPver::v4) {
                IN_ADDR v4{};
                if (InetPtonA(AF_INET, strAddress.c_str(), &v4) != 1) { std::free(address); address=nullptr; throw std::invalid_argument("invalid IPv4"); }
                memcpy(address, &v4, 4);
            } else {
                IN6_ADDR v6{};
                if (InetPtonA(AF_INET6, strAddress.c_str(), &v6) != 1) { std::free(address); address=nullptr; throw std::invalid_argument("invalid IPv6"); }
